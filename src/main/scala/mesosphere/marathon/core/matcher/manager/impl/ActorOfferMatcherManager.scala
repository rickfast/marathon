...
private[matcher] class ActorOfferMatcherManager(actorRef: ActorRef) extends OfferMatcherManager {
  ...
  override def addSubscription(offerMatcher: OfferMatcher)(implicit ec: ExecutionContext): Future[Unit] = {
    val future = actorRef ? ActorOfferMatcherManager.AddOrUpdateMatcher(offerMatcher)
    future.map(_ => ())
  }
  ...
  override def addLaunchTokens(tokens: Int): Unit = actorRef ! ActorOfferMatcherManager.AddTaskLaunchTokens(tokens)
  override def setLaunchTokens(tokens: Int): Unit = actorRef ! ActorOfferMatcherManager.SetTaskLaunchTokens(tokens)
}
