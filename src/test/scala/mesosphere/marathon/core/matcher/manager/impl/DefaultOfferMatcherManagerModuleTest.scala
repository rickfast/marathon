...
class DefaultOfferMatcherManagerModuleTest extends FunSuite with BeforeAndAfter {
  ...
  before {
    ...
    config.afterInit()
    module = OfferMatcherManagerModule(clock, random, new Metrics(new MetricRegistry), config, actorSystem)
  }
  ...
  private class ConstantOfferMatcher(tasks: Seq[TaskInfo]) extends OfferMatcher {
    ...
    object source extends TaskLaunchSource {
      ...
      override def taskLaunchRejected(taskInfo: TaskInfo): Unit = rejectedTasks :+= taskInfo
    }
  }
